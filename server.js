...
function applyProjectDeadlinePenalties() {
  ...
  Object.values(projects).forEach((proj) => {
    ...
    if (dailyPenaltyPct > 0) {
      totalPenalty += dailyPenaltyPct;
      appendEventLog(shared,{
        day: currentDay,
        message: `Project deadline penalty (${proj.name} – ${level}): −${dailyPenaltyPct}% City Health`
      });
    }
  });
...
function applyInventoryHoldingAndDegradation() {
  ...
  ['municipality', 'mrf', 'broker'].forEach((rk) => {
    ...
    holdingCost += total * (HOLDING_COST_HKD_PER_TON_PER_DAY[rk] || 0);
  });
  if (holdingCost > 0) {
  ...
  if (currentDay % DEGRADE_EVERY_DAYS !== 0) return;
  ['municipality', 'mrf', 'broker'].forEach((rk) => {
    const inv = roles[rk]?.inventory;
    if (!inv) return;
    ...
    if (inv.materials) {
      Object.values(inv.materials).forEach((grades) => {
        const a = grades.A || 0;
        const b = grades.B || 0;
        const moveA = Math.floor(a * DEGRADE_RATE);
        const moveB = Math.floor(b * DEGRADE_RATE);
    ...
    if (inv.waste) {
      Object.values(inv.waste).forEach((grades) => {
        ...
        const c = grades.C || 0;
        const moveB = Math.floor(b * DEGRADE_RATE);
        const moveC = Math.floor(c * DEGRADE_RATE);
        if (moveB > 0) { grades.B -= moveB; grades.C = (grades.C || 0) + moveB; }
        if (moveC > 0) { grades.C -= moveC; grades.F = (grades.F || 0) + moveC; }
      });
    }
  });
  appendEventLog(shared,{ day: currentDay, message: `Inventory quality degraded: ${Math.round(DEGRADE_RATE * 100)}% A→B→C (materials) and B→C→F (waste) (every ${DEGRADE_EVERY_DAYS} days)` });
}
...
function checkGameOver() {
  ...
  broadcastGameState();
  io.emit('gameOver', { reason, state: buildClientGameStatePayload(gameState) });
}
...
io.on('connection', (socket) => {
  console.log(`⚡ Socket connected: ${socket.id}  (current count: ${players.length})`);

  // Defer join so any pending "disconnect" from a tab refresh runs first (avoids false "room full").
  // Also prune stale player rows whose socket is already gone from the server map.
  setImmediate(() => {
    players = players.filter((p) => io.sockets.sockets.has(p.id));

    if (players.length >= MAX_PLAYERS) {
      console.log(`🚫 Rejecting ${socket.id} — room full (${players.length}/${MAX_PLAYERS})`);
    ...
    console.log(`✅ Player added. Total: ${players.length}/${MAX_PLAYERS}`);
  ...
  socket.on('collectWasteFromDistrict', (data) => {
    ...
    const co2Pct = Math.round((co2Kg * CO2_KG_TO_PERCENT) * 10) / 10;
  ...
  socket.on('projectSubmitMaterial', (data) => {
    ...
    const co2Pct = Math.round((fracCo2Kg * CO2_KG_TO_PERCENT) * 10) / 10;
    shared.carbonEmissions.value = Math.min(
      shared.carbonEmissions.limit,
  ...
  socket.on('projectSubmitAll', (data) => {
    ...
    const fracHealth = cfg.healthGainPct / n;
    const fracCo2Kg = cfg.co2ImpactKg / n;
    const co2PctEach = Math.round((fracCo2Kg * CO2_KG_TO_PERCENT) * 10) / 10;
  ...
  socket.on('brokerSendToMunicipality', (data) => {
    ...
    const transportCo2Pct = Math.round((av.cfg.co2PerTrip * transportUnits * CO2_KG_TO_PERCENT) * 10) / 10;
