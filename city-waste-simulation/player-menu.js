  ...
  const DEFAULT_MENUS = {
    ...
    Broker: {
      ...
      items: [
        ...
        { label: 'Transportation', href: 'broker-transportation.html' },
      ],
    },
  };
...
(function () {
  function inferGameOverReason(shared) {
    ...
    const hit = [...shared.eventLog]
      .reverse()
      .find((e) => e && e.message && String(e.message).includes('Game Over'));
    if (!hit) return '';
    const m = String(hit.message).match(/Game Over:\s*(.+)$/i);
    return m ? m[1].trim() : '';
  }
  ...
  function ensureGameOverModal() {
    ...
    overlay.innerHTML = `
      <div class="max-w-md w-full rounded-2xl border-2 border-[#2f5a47] bg-white shadow-2xl p-6 text-center">
        <h2 id="besseGameOverTitle" class="text-2xl font-extrabold tracking-wide text-[#1a2f25] mb-2">Game Over</h2>
...
(function () {
  ...
  function applyPlayerNameToHeader(displayName) {
    ...
    if (headerName && role) {
      headerName.textContent = `${displayName}, ${role}`;
      return;
    }
    if (headerName) {
